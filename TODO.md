...
### 主从分工 / 行为契约
- [ ] 子 AI 对简单任务过度侦查：实测"创建对象"消耗 53 个工具调用。
  - 发现场景：2026-09-05 实机验证 Forge→Veritas 链路，主 AI 派
    子 AI 创建世界对象。理想链路只需 4 个工具（world_info →
    create_object → get_world_object → 报告），实际消耗 53 个：
    大量时间花在查文档、读代码、反复核实对象模型是否有
    type/name 字段、确认 create_object 参数。
  - 根因：SUBAGENT_SYSTEM 要求"用工具探索"+"写 EVIDENCE 必须
    引用真实 tool_call_id"，但没有"简单任务限制侦查步数"的约束。
    子 AI 对简单任务和复杂任务用同一套谨慎策略。
  - 影响：token 浪费严重；用户等待时间远超必要；审计日志噪音大。
  - 建议：子 AI prompt 增加"任务复杂度预估"——简单任务限制
    侦查步数（如 ≤5），复杂任务才允许充分探索。或引入工具调用
    预算上限，超了返回 need_decision 让主 AI 判断是否继续。
  - 优先级：P1
